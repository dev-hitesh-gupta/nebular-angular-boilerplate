...
import {STATUS_CODE} from '@athlete-first/core/api/error-code';
...
export class ErrorInterceptor implements HttpInterceptor {
  ...
  private _checkTokenExpiryErr(error: HttpErrorResponse): boolean {
    return (
      ...
      get(error, 'error.error.message.name') === 'TokenExpiredError'
    );
  }
      ...
      return next.handle(req).pipe(
        catchError((error, caught) => {
          if (
            ...
            errMsg =
              errMsg ||
              get(error, error.error.message, 'Invalid User Credentials');
            this.toastrService.error(errMsg, 'ERROR !', {
              timeOut: environment.messageTimeout,
            });
          } else {
