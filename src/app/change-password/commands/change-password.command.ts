...
import {PatchAPICommand} from '@athlete-first/core/core.module';
import {environment} from '@athlete-first/env/environment';

export class ChangePasswordCommand<T> extends PatchAPICommand<T> {
  constructor(apiService: ApiService, adapter: IAdapter<T>) {
    super(apiService, adapter, `${environment.authApiUrl}/`);
  }
}
